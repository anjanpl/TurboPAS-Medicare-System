...
' Purpose/Function: Discharge Reason Setup 
...
Namespace Visitel.UserControl.Settings
    Public Class DischargeReasonSetting
        ...
        Private ControlName As String, EditText As String, DeleteText As String, InsertMessage As String, UpdateMessage As String, DeleteMessage As String, _
         DuplicateDischargeReasonNameMessage As String, EmptyDischargeReasonNameMessage As String, StatusSelectionMessage As String

        Protected Sub Page_Init(sender As Object, e As EventArgs)
            ControlName = "DischargeReasonSetting"
        ...
        Protected Sub ButtonDelete_Click(sender As Object, e As EventArgs)
            ...
            Dim objBLDischargeReasonSetting As New BLDischargeReasonSetting()
            Try
                objBLDischargeReasonSetting.DeleteDischargeReasonInfo(ConVisitel, Convert.ToInt32(LabelIdNumber.Text), UserId)
            ...
            Catch ex As Exception
                If (ex.Message.Contains("REFERENCE constraint")) Then
                    DirectCast(Me.Page.Master, IMyMasterPage).DisplayHeaderMessage(String.Format("Unable to delete Discharge Reason Data. Message: {0}",
                                                                                                 "This Discharge Reason is already used."))
                Else
                    DirectCast(Me.Page.Master, IMyMasterPage).DisplayHeaderError(String.Format("Unable to delete Discharge Reason Data. Message: {0}", ex.Message))
                End If
            Finally
                objBLDischargeReasonSetting = Nothing
            End Try

            BindDischargeReasonInformationGridView()
            HiddenFieldIsNew.Value = Convert.ToString(False, Nothing)
        ...
        Private Sub ButtonSave_Click(sender As Object, e As EventArgs)
            If ValidateData() Then
                Dim objDischargeReasonSettingDataObject As New DischargeReasonSettingDataObject()

                objDischargeReasonSettingDataObject.Name = Convert.ToString(TextBoxName.Text, Nothing).Trim()
                objDischargeReasonSettingDataObject.Status = Convert.ToInt16(DropDownListStatus.SelectedValue)

                Dim objBLDischargeReasonSetting As New BLDischargeReasonSetting()

                Select Case Convert.ToBoolean(HiddenFieldIsNew.Value, Nothing)
                    Case True
                        objDischargeReasonSettingDataObject.UserId = UserId
                        objDischargeReasonSettingDataObject.CompanyId = CompanyId
                        Try
                            objBLDischargeReasonSetting.InsertDischargeReasonInfo(ConVisitel, objDischargeReasonSettingDataObject)

                        Catch ex As SqlException
                            If ex.Message.Contains("Violation of UNIQUE KEY constraint 'UQ_DischargeReasonName'") Then
                                DirectCast(Me.Page.Master, IMyMasterPage).DisplayHeaderMessage(DuplicateDischargeReasonNameMessage)
                                Return
                            End If
                        End Try
                        ...
                        Exit Select
                    Case False
                        objDischargeReasonSettingDataObject.IdNumber = Convert.ToInt32(HiddenFieldIdNumber.Value)
                        objDischargeReasonSettingDataObject.UpdateBy = Convert.ToString(UserId)
                        Try
                            objBLDischargeReasonSetting.UpdateDischargeReasonInfo(ConVisitel, objDischargeReasonSettingDataObject)
                        Catch ex As SqlException
                            If ex.Message.Contains("Violation of UNIQUE KEY constraint 'UQ_DischargeReasonName'") Then
                                DirectCast(Me.Page.Master, IMyMasterPage).DisplayHeaderMessage(DuplicateDischargeReasonNameMessage)
                                Return
        ...
        Private Sub GridViewDischargeReasonInformation_PageIndexChanging(sender As Object, e As GridViewPageEventArgs)
            GridViewDischargeReasonInformation.PageIndex = e.NewPageIndex
            BindDischargeReasonInformationGridView()
        End Sub

        Private Sub GridViewDischargeReasonInformation_RowDataBound(sender As [Object], e As GridViewRowEventArgs)
        ...
        Private Sub InitializeControl()
            ...
            AddHandler GridViewDischargeReasonInformation.PageIndexChanging, AddressOf GridViewDischargeReasonInformation_PageIndexChanging
            AddHandler GridViewDischargeReasonInformation.RowDataBound, AddressOf GridViewDischargeReasonInformation_RowDataBound

            GridViewDischargeReasonInformation.AutoGenerateColumns = False
            GridViewDischargeReasonInformation.ShowHeaderWhenEmpty = True
            GridViewDischargeReasonInformation.AllowPaging = True
            GridViewDischargeReasonInformation.PageSize = objShared.GridViewDefaultPageSize
        ...
        Private Sub LoadJavascript()
            ...
            scriptBlock = "<script type='text/javascript'> " _
                              & " var DeleteDialogHeader ='Discharge Reason'; " _
        ...
        Private Sub GetCaptionFromResource()
            ...
            LabelDischargeReasonInformationEntry.Text = Convert.ToString(ResourceTable("LabelDischargeReasonInformationEntry"), Nothing)
            LabelDischargeReasonInformationEntry.Text = If(String.IsNullOrEmpty(LabelDischargeReasonInformationEntry.Text),
                                                           "Discharge Reason Information Entry", LabelDischargeReasonInformationEntry.Text)

            LabelSearchDischargeReasonInfo.Text = Convert.ToString(ResourceTable("LabelSearchDischargeReasonInfo"), Nothing)
            LabelSearchDischargeReasonInfo.Text = If(String.IsNullOrEmpty(LabelSearchDischargeReasonInfo.Text),
                                                     "Search Discharge Reason Information", LabelSearchDischargeReasonInfo.Text)

            LabelDischargeReasonInformationList.Text = Convert.ToString(ResourceTable("LabelDischargeReasonInformationList"), Nothing)
            LabelDischargeReasonInformationList.Text = If(String.IsNullOrEmpty(LabelDischargeReasonInformationList.Text),
                                                          "Discharge Reason Information List", LabelDischargeReasonInformationList.Text)
            ...
            DuplicateDischargeReasonNameMessage = Convert.ToString(ResourceTable("DuplicateDischargeReasonNameMessage"), Nothing)
            DuplicateDischargeReasonNameMessage = If(String.IsNullOrEmpty(DuplicateDischargeReasonNameMessage),
                                                     "The same reason already exists.", DuplicateDischargeReasonNameMessage)

            EmptyDischargeReasonNameMessage = Convert.ToString(ResourceTable("EmptyDischargeReasonNameMessage"), Nothing)
            EmptyDischargeReasonNameMessage = If(String.IsNullOrEmpty(EmptyDischargeReasonNameMessage), "Name Cannot be Blank", EmptyDischargeReasonNameMessage)

            StatusSelectionMessage = Convert.ToString(ResourceTable("StatusSelectionMessage"), Nothing)
            StatusSelectionMessage = If(String.IsNullOrEmpty(StatusSelectionMessage), "Select status...", StatusSelectionMessage)
        ...
        Private Function ValidateData() As Boolean
            If String.IsNullOrEmpty(TextBoxName.Text.Trim()) Then
                DirectCast(Me.Page.Master, IMyMasterPage).DisplayHeaderMessage(EmptyDischargeReasonNameMessage)
                Return False
            End If
        ...
        Private Sub GetData()
            ...
            objShared.BindStatusDropDownList(DropDownListSearchByStatus)
            BindDischargeReasonInformationGridView()
        End Sub
        ...
        ''' <summary>
        ''' Binding Discharge Reason Information GridView
        ''' </summary>
        Private Sub BindDischargeReasonInformationGridView()

            Dim DischargeReasonSettingList As List(Of DischargeReasonSettingDataObject)

            Dim objBLDischargeReasonSetting As New BLDischargeReasonSetting()

            DischargeReasonSettingList = If((Convert.ToBoolean(HiddenFieldIsSearched.Value, Nothing)),
                                              objBLDischargeReasonSetting.SearchDischargeReasonInfo(ConVisitel, CompanyId, TextBoxSearchByName.Text.Trim(),
                                                                      Convert.ToInt16(DropDownListSearchByStatus.SelectedValue)),
                                              objBLDischargeReasonSetting.SelectDischargeReasonInfo(ConVisitel, CompanyId))

            objBLDischargeReasonSetting = Nothing

            GridViewDischargeReasonInformation.DataSource = DischargeReasonSettingList
            GridViewDischargeReasonInformation.DataBind()

            DischargeReasonSettingList = Nothing
        End Sub
    End Class
End Namespace